    ...
    if instance:IsA("Model") then
        for _, p in ipairs(instance:GetChildren()) do
            if p:IsA("BasePart") then
                local size = (p.Size / 2) * library.box.padding
                local cf = p.CFrame
            ...
            elseif p:IsA("Accessory") then
                ...
                if handle and handle:IsA("BasePart") then
                    local size = (handle.Size / 2) * library.box.padding
    ...
    for instance, data in pairs(espinstances) do
        ...
        if data.box then
            ...
            if library.box.enabled and onscreen then
                ...
                elseif library.box.type == "corner" then
                    local fill_lines = box.corner_fill
                    local outline_lines = box.corner_outline
                    local fill_color = library.box.fill
                    local outline_color = library.box.outline
        ...
        if data.healthbar then
            ...
            if not library.healthbar.enabled or not onscreen then
                outline.Visible = false
                fill.Visible = false
            else
        ...
        if data.name then
            if library.name.enabled and onscreen then
                ...
                text.Text = name_str
                text.Size = library.name.size
                text.Color = library.name.fill
        ...
        if data.distance then
            if library.distance.enabled and onscreen then
                ...
                text.Size = library.distance.size
                text.Color = library.distance.fill
                text.Position = Vector2.new(center_x, y)
                text.Visible = true
            else
        ...
        if data.tracer then
            if library.tracer.enabled and onscreen then
                ...
                elseif library.tracer.from == "bottom" then
                    from_pos = Vector2.new(camera.ViewportSize.X/2, camera.ViewportSize.Y)
                elseif library.tracer.from == "center" then
                ...
                outline.To = to_pos
                outline.Color = library.tracer.outline
                outline.Visible = true

                fill.From = from_pos
                fill.To = to_pos
                fill.Color = library.tracer.fill
                fill.Visible = true
            else
